...
import { selectItem, removeItem } from "../../redux/slices/selectionSlice";
...
const Page = ({ name, level, image }) => {
  ...
  const handleClick = ({ name }) => {
    if (!selected_items.includes(name)) dispatch(selectItem(name));
        ...
        <>
          <div
            ...
            className={`flex flex-wrap cursor-pointer justify-center items-center w-30 h-30 m-1 border-2 rounded-xl ${
              selected_items.includes(name) ? `bg-[#305c4c]` : "bg-[#1C4464] hover:bg-blue-950"
