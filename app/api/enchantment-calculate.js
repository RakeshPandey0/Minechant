...
const merge = (target, sacrifice) => {
  ...
  for (const [name, sacLevel] of Object.entries(sacrifice.enchantments)) {
    ...
    const isIncompatible =
      Object.keys(target.enchantments).some((t) => data.incompatible.has(t)) ||
      Object.keys(target.enchantments).some(
        (t) => ENCHANTMENT_REGISTRY[t]?.incompatible.has(name)
      );
  ...
  const isUseful =
    ...
    (target.type === "book" && sacrifice.type === "book" && enchantCost > 0) ||
    hasChanges;
...
export const calculateEnchantmentPlan = (baseItemName, selectedEnchants) => {
  ...
  if (!finalNode) {
    return { error: "No valid plan possible under 39 levels per operation." };
  }
